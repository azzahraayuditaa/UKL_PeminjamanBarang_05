...
export const returnBarang = async (request: Request, response: Response) => {
    try {
        ...
        // Ambil peminjaman dengan borrow_date
        const peminjaman = await prisma.peminjaman.findUnique({
            ...
            select: { 
                ...
                status: true, 
                id_barang: true,
                borrow_date: true // Ambil borrow_date
            },
        });
        ...
        const borrowDate = new Date(peminjaman.borrow_date);
        const returnDate = new Date(return_date);

        // Memastikan return_date tidak lebih awal dari borrow_date
        ...
        // Format tanggal menjadi ISO string
        const formattedReturnDate = returnDate.toISOString(); // Menggunakan toISOString untuk mendapatkan format yang benar

        const updatedPeminjaman = await prisma.peminjaman.update({
            where: { id_peminjaman: Number(borrow_id) },
            data: {
                return_date: formattedReturnDate, // Menggunakan formattedReturnDate
                status: status,
            },
        });
...
export const analisis = async (request: Request, response: Response) => {
    ...
    try {
        ...
        const usageAnalysis = usageReport.map(item => {
            ...
            const totalReturned = item._sum.quantity ?? 0; // Jika qty null, gunakan 0
...
export const deleteBarang = async (request: Request, response: Response) => {
    try {
        ...
        return response.status(200).json({
            status: true,
            message: `Barang dengan ID ${id_barang} telah dihapus`,
        });
    } catch (error) {
        response.status(500).json({
            status: "error",
